...
class TestLoadConfig:
    ...
    def agency_dir(self, tmp_path: Path) -> None:
    ...
    def test_overrides_from_yaml(self, tmp_path: Path) -> None:
        (tmp_path / ".agency" / "config.yml").write_text(
            "max_iterations: 10\ncontext: My project\n"
        )
        config = load_config(tmp_path)
    ...
    def test_null_values_fall_back_to_defaults(self, tmp_path: Path) -> None:
        (tmp_path / ".agency" / "config.yml").write_text("max_iterations: null\ncontext: null\n")
        config = load_config(tmp_path)
        assert config.max_iterations == 5
        assert config.context == ""

    def test_unknown_keys_ignored(self, tmp_path: Path) -> None:
        (tmp_path / ".agency" / "config.yml").write_text("max_iterations: 3\nunknown_key: hello\n")
        config = load_config(tmp_path)
    ...
    def test_invalid_type_for_int_field(self, tmp_path: Path) -> None:
        (tmp_path / ".agency" / "config.yml").write_text("max_iterations: five\n")
    ...
    def test_invalid_type_for_str_field(self, tmp_path: Path) -> None:
        (tmp_path / ".agency" / "config.yml").write_text("context: 42\n")
    ...
    def test_invalid_type_for_optional_str_field(self, tmp_path: Path) -> None:
        (tmp_path / ".agency" / "config.yml").write_text("analyze_prompt: [1, 2, 3]\n")
